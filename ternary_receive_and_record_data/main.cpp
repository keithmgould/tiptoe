...
void demodulator(const void * inputBuffer, paTestData * data, vector<bool>& bits)
{
  ...
  {
    if(deltas.at(i) > 0.000416)
    {
      trits.push_back(1);
    }else if(deltas.at(i) < 0.000333)
    {
      trits.push_back(5);
    }else{
      trits.push_back(3);
...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i < data.trits.size(); i++)
    {
      for(int j=0; j < data.trits.at(i).size(); j++)
      {
        cout << data.trits.at(i).at(j) << ",";
      }
      cout << endl;
    }
